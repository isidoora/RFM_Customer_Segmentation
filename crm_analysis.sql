/* analyzing what customer's characteristics lead to acceptance of campaign */
/* target variable pilot_campaign has value 1 if the customer positively engaged with the campaign, 0 otherwise */
...
/* pilot campaign success by gender */
SELECT
	gender,
    	COUNT(*)
FROM ifood
...
/* checking marital status classification */
...
FROM ifood
GROUP BY 1;
-- taken spend more in general and were a lot more influenced by all previous campaigns but not this one
...
/* through which channel should we aim the next campaign? */
SELECT
...
(
	SELECT
		...
		ROUND((recency + frequency + monetary_value)/3) AS rfm_category
	FROM
    	(
		SELECT
			id,
			NTILE(5) OVER(ORDER BY last_purchase DESC) AS recency,
			NTILE(5) OVER(ORDER BY count_purchases) AS frequency,
...
FROM rfm
	LEFT JOIN ifood 
		ON rfm.id = ifood.id
WHERE ifood.pilot_campaign = 1
GROUP BY 1
ORDER BY 2;

-- ----------------------------- ------------------------------- --------------------------- ------------------------------- ------------------------------ --
/* some queries used to gain insights into data
that were irrelevant for inclusion in the final report */


/* spending across food categories */
SELECT
	SUM(wines) OVER() AS wines,
...
/* gold/regular */
SELECT
	SUM(gold_products) OVER() AS gold,
